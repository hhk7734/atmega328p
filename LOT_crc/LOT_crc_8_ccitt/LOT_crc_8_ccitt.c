/**
 * @file LOT_crc_8_ccitt.c
 * @author Hyeon-ki, Hong (hhk7734@gmail.com)
 * @brief CRC-8-CCITT, X^8 + X^2 + X + 1
 */

#include "LOT_crc_8_ccitt.h"
#ifndef _LOT_USE_CRC_8_CCITT_TABEL_
#    include <util/crc16.h>
#else
#    include <avr/pgmspace.h>

/// @brief CRT-8-CCITT 테이블
PROGMEM static const uint8_t LOT_crc_8_ccitt_table[]
...
/**
 * @brief data를 사용하여 CRC-8-CCITT 
 * @param uint8_t *data 검사할 데이터
 * @param uint8_t count 검사할 데이터 수
 * @return 송신 시 FCS로 사용할 값 반환, 수신 시 0 이면 정상
 */
uint8_t LOT_crc_8_ccitt_check( const uint8_t *data, uint8_t count )
{
    uint8_t crc = LOT_CRC_8_CCITT_INIT;
    while ( count-- )
    {
#ifdef _LOT_USE_CRC_8_CCITT_TABEL_
        crc = pgm_read_byte( &LOT_crc_8_ccitt_table[crc ^ *data++] );
#else
